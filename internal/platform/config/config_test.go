...
func TestLoadDefaults(t *testing.T) {
	...
	t.Setenv("GOOGLE_HEALTH_HTTP_CLIENT_TIMEOUT_SECONDS", "")
	...
	if cfg.HTTPClientTimeout != 5*time.Second || cfg.GoogleHealthTimeout != 30*time.Second || cfg.AccessTokenExpiresIn != 24*time.Hour {
...
func TestLoadOverridesAndInvalidDurations(t *testing.T) {
	...
	t.Setenv("GOOGLE_HEALTH_HTTP_CLIENT_TIMEOUT_SECONDS", "45")
	t.Setenv("ACCESS_TOKEN_EXPIRES_SECONDS", "bad")

	cfg := Load("api", "8080")
	if cfg.Port != "9000" || cfg.JWTSecret != "secret" || cfg.HTTPClientTimeout != 12*time.Second || cfg.GoogleHealthTimeout != 45*time.Second {
