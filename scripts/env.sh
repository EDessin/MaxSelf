...
ENV_FILE="${MAXSELF_ENV_FILE:-${PROJECT_ROOT}/.env}"
...
# Local scripts run Go services on the host, while Docker Compose uses service
# DNS names such as postgres, identity, activity, and progress.
DATABASE_URL="${DATABASE_URL/@postgres:/@localhost:}"
IDENTITY_SERVICE_URL="${IDENTITY_SERVICE_URL/identity:8081/localhost:8081}"
ACTIVITY_SERVICE_URL="${ACTIVITY_SERVICE_URL/activity:8082/localhost:8082}"
PROGRESS_SERVICE_URL="${PROGRESS_SERVICE_URL/progress:8083/localhost:8083}"
